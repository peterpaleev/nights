...
export function main(coord, context, cursor, buffer) {
    ...
    let color
    if (u > 30) {
        color = frequencyToColor(frequencyData)
    } else if (u > 20) {
        color = ''
    } else {
        color = 'white'
...
function frequencyToColor(frequencyData) {
    const lowFreq = frequencyData.slice(0, frequencyData.length / 3).reduce((a, b) => a + b, 0)
    const midFreq = frequencyData.slice(frequencyData.length / 3, 2 * frequencyData.length / 3).reduce((a, b) => a + b, 0)
    const highFreq = frequencyData.slice(2 * frequencyData.length / 3).reduce((a, b) => a + b, 0)
